...
errorcount <- errors %>% 
  ...
  summarise(error1sum = sum(errorID==1), 
            error2sum = sum(errorID==2), 
            error3sum = sum(errorID==3), 
            error4sum = sum(errorID==4), 
            error5sum = sum(errorID==5)) %>%
  ungroup()
...
comprep <- maint %>% 
    ...
    mutate(comp1 = as.integer(comp == 1), 
           comp2 = as.integer(comp == 2),
           comp3 = as.integer(comp == 3),
           comp4 = as.integer(comp == 4)) %>%
    select(-comp)
